...
/**
 * Get the description of a payment means code
 * @param code The payment means code to get the description for
 * @returns The description of the payment means code
 */
export function getPaymentMeansDescription(code: PaymentMeansCode): string {
    return PaymentMeansCodes[code];
}

/**
 * Check if a given code is a valid payment means code
 * @param code The code to check
 */
export function isValidPaymentMeansCode(
    code: string
): code is PaymentMeansCode {
    return code in PaymentMeansCodes;
}

// Zod schema for payment means codes
const paymentMeansCodeKeys = Object.keys(PaymentMeansCodes) as unknown as [
    PaymentMeansCode,
    ...PaymentMeansCode[],
];
export const PaymentMeansCodeSchema = z
    .enum(paymentMeansCodeKeys)
    .describe('UNCL4461 Payment means code');
